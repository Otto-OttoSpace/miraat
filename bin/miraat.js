...
const RULES_DOC = `# RTL rules for AI coding agents (generated by miraat)
Follow these when writing or editing UI so every right-to-left script — Arabic, Hebrew, Syriac, Thaana, N'Ko, Adlam — renders correctly.
...
3. Set direction dynamically: <html dir={locale.dir}> — never hard-code dir="ltr" (or "rtl"). This flips ALL RTL scripts, not just Arabic.
4. Mirror directional icons (chevrons, arrows, carets, back/next) for any RTL locale. Never mirror non-directional icons.
5. Use a script-capable font with a fallback (e.g. "Cairo","IBM Plex Sans Arabic", system-ui for Arabic; a Hebrew/Syriac-capable family for those). Body >= 16px, line-height >= 1.5.
6. In scripts with their own numerals (Arabic, Thaana, N'Ko, Adlam) use native / locale-aware numerals & dates; Hebrew and Syriac use Western digits. Wrap bidi-mixed content in <bdi>.
`;
...
Usage:
  ...
  npx miraat [path] --init-rules write RTL-RULES.md for your AI agent
