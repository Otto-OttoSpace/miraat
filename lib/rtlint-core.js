...
const MSG = {
  ...
  icon: 'directional icon — mirror it for RTL (or use a logical/auto-flipping icon)',
  dir: 'hard-coded direction — make it dynamic (e.g. dir={locale.dir})',
  digits: 'Western digits in Arabic text — use Arabic-Indic numerals / locale-aware formatting',
...
function scanJs(file, src, ext, findings, edits) {
  ...
  const arabicDigits = (text, line) => {
    if (!ARABIC_RE.test(text) || !/[0-9]/.test(text)) return false;
  ...
  babelTraverse(ast, {
    ...
    TemplateElement(p) {
      ...
      arabicDigits(raw, line);
    },
    JSXText(p) {
      const n = p.node;
      const line = n.loc.start.line;
      if (arabicDigits(n.value, line)) return;
      // lang="ar" / dir="rtl" ancestor: flag Western digits even without Arabic glyphs
...
function scanMarkup(file, src, findings, edits) {
  ...
  // Arabic + Western digits, line heuristic
  const lines = src.split('\n');
  for (let i = 0; i < lines.length; i++) {
    const ln = lines[i];
    const hasArabic = ARABIC_RE.test(ln);
    const rtlEl = /\blang\s*=\s*["']?ar\b/i.test(ln) || /\bdir\s*=\s*["']?rtl\b/i.test(ln);
    if ((hasArabic || rtlEl) && /[0-9]/.test(ln))
