...
// True when byte index `at` sits inside an <input type="tel|email|url|number"> tag.
// Used so hardcoded-dir doesn't flag the exact dir="ltr" that input-dir prescribes.
function inLtrNativeInput(src, at) {
  const lt = src.lastIndexOf('<', at);
  if (lt < 0) return false;
  let gt = src.indexOf('>', at);
  if (gt < 0) gt = src.length;
  const tag = src.slice(lt, gt + 1);
  return /^<input\b/i.test(tag) && INPUT_TYPE_RE.test(tag);
}
  ...
  if (opts.dir !== false) {
    for (const re of [DIR_ATTR_RE, DIR_CSS_RE, DIR_SETATTR_RE]) {
      ...
      while ((m = re.exec(src)) !== null) {
        // The dir="ltr" that input-dir itself prescribes on an LTR-native input
        // (tel/email/url/number) is correct, not a bug — the two rules must agree.
        if (re === DIR_ATTR_RE && m[1] && m[1].toLowerCase() === 'ltr'
            && inLtrNativeInput(src, m.index)) continue;
        push('hardcoded-dir', lineAt(src, m.index), m[0].trim(), MSG.dir);
      }
    }
